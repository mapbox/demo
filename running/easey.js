;(function(context, MM) {
    var easey = function() {
        ...
        // * time
        // * path
        var from, to, map, angle, startangle, startscale, scale;
        ...
        easey.sangle = function(x) {
            startangle = x;
            return easey;
        };

        easey.angle = function(x) {
            angle = x;
            return easey;
        };

        easey.scale = function(x) {
            scale = x;
            return easey;
        };
        ...
        easey.run = function(time, callback) {
            ...
            if (typeof startangle == 'undefined') startangle = map.angle;
            if (typeof startscale == 'undefined') startscale = map.scale;
            if (typeof angle == 'undefined') angle = map.angle;
            if (typeof scale == 'undefined') scale = map.scale;
            if (!to) to = map.coordinate.copy();
            time = time || 1000;
            start = (+new Date());
            running = true;
            ...
            function tick() {
                ...
                if (abort) {
                    ...
                    var d = (angle + startangle + 360) % 360 > 180 ? 1 : -1;
                    var d = 1;
                    var del = easing(delta / time);
                    var delan = ((angle - startangle + 540) % 360) - 180;
                    var an = d * delan * del + startangle;
                    an = (an + 360)  % 360;
                    var sc = (scale - startscale) * del + startscale;
                    map.transform(
                            an,
                            sc,
                            null
